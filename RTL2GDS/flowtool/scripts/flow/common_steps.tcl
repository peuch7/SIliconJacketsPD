...
create_flow_step -name signoff_finish -owner cadence -categories flow {
  apply {{} {
    if {[is_flow -inside flow:signoff] } {
      set stage opt_signoff
    }
    set signoff_report_dir [file dirname [file normalize [get_db flow_db_directory]]]
    set signoff_report_dir [file normalize [file join $signoff_report_dir ${stage}_outputs]]
    if {![file exists $signoff_report_dir]} {
      file mkdir [file normalize $signoff_report_dir]
    }
    check_design -type all
    check_design -type all -out_file ${signoff_report_dir}/check_design_all_report.rpt
    report_timing_summary
    report_timing_summary > ${signoff_report_dir}/timing_summary_report.rpt
  }}
}
...
# STEP innovus_output
##############################################################################
create_flow_step -name innovus_output -owner cadence {
  apply {{} {
    if {[is_flow -inside flow:postroute]} {
      set stage postroute
    } elseif {[is_flow -inside flow:opt_signoff] } {
      set stage opt_signoff
    }
    set golden_dir [file dirname [file normalize [get_db flow_db_directory]]]
    set golden_dir [file normalize [file join $golden_dir ${stage}_outputs]/golden]
#    set golden_dir [file join [get_db flow_db_directory] [get_db flow_report_name]/golden]
    set DESIGN_NAME [get_db designs .name]
    # Write SPEF
    if {[is_flow -inside flow:postroute]} {
      extract_rc
      set rc_corners [get_db rc_corners .name]
      foreach obj $rc_corners {
        write_parasitics -rc_corner $obj -spef_file $golden_dir/golden_${DESIGN_NAME}_${obj}.spef
      }
    }    
    # Write LEF 
    write_lef_abstract $golden_dir/golden_${DESIGN_NAME}.lef  -stripe_pins -pg_pin_layers 5 -top_layer 5
    # Write GDS
    write_stream $golden_dir/golden_${DESIGN_NAME}.gds.gz -merge { resources/sram_sky130/gds/sky130_sram_2kbyte_1rw1r_32x512_8.gds resources/cadence_sky130/sky130_scl_9T_0.1.1/sky130_scl_9T/gds/sky130_scl_9T.gds resources/cadence_sky130/sky130_scl_9T_0.1.1/sky130_scl_9T_tech/gds/sky130_scl_9T_tech.gds } -map_file resources/cadence_sky130/sky130_scl_9T_0.1.1/sky130_scl_9T/gds/sky130_stream.mapFile -unit 1000
    # Write netlist
    write_netlist $golden_dir/design.v -top_module ${DESIGN_NAME} -phys -include_pg_ports -exclude_leaf_cells
    write_netlist $golden_dir/leaf_cell.v -only_leaf_cells -include_pg_ports
    tail -n +10 $golden_dir/leaf_cell.v > $golden_dir/tmp.v
    sed "9r $golden_dir/tmp.v" $golden_dir/design.v > $golden_dir/golden_${DESIGN_NAME}.v
    rm $golden_dir/leaf_cell.v $golden_dir/tmp.v $golden_dir/design.v
    # Write sim netlist
    write_netlist $golden_dir/golden_${DESIGN_NAME}_sim.v -top_module ${DESIGN_NAME} -exclude_leaf_cells
    # Write DEF
    write_def -all_layers $golden_dir/golden_${DESIGN_NAME}.def
    # Write SDF
    set func_views [get_db analysis_views .name]
    foreach obj $func_views {
      write_sdf -recompute_delaycal -precision 4 -view $obj $golden_dir/golden_${DESIGN_NAME}_${obj}.sdf
    }

    report_summary -out_dir $golden_dir/golden_${DESIGN_NAME}_Summary
  }}
}
