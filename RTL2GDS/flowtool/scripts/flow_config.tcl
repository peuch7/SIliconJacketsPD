...
# FLOW SUSPEND 
################################################################################
create_flow_step -name suspend_flow -owner flow {
  suspend
}
...
################################################################################
create_flow_step -name initial_sanity_checks -owner flow {
  set design  [get_db current_design .name]
  check_design -lib_lef_consistency
  check_design design:${design} -unresolved
  report_gates  > [file join [get_db flow_report_directory] [get_db flow_report_name] ${design}_elab_gates.rpt]
  report_module > [file join [get_db flow_report_directory] [get_db flow_report_name] ${design}_elab_module.rpt]
  report timing -lint -verbose > [file join [get_db flow_report_directory] [get_db flow_report_name] ${design}_tim_lint.rpt]
}
################################################################################
...
create_flow_step -name init_mcpu -owner flow {
  apply {{} {
    ...
    switch -glob [get_db program_short_name] {
      default       {}
      joules*       -
      genus*        { set_multi_cpu_usage -verbose -local_cpu $max_cpus }
      innovus*      { set_multi_cpu_usage -verbose -local_cpu $max_cpus }
      tempus*       { set_multi_cpu_usage -verbose -local_cpu $max_cpus }
...
if {[get_feature report_innovus]} {
  create_flow_step -name genus_to_innovus -owner flow {
    apply {{} {
      set design [get_db current_design .name]
      set innov_dir [file join [get_db flow_db_directory] ["INNOVUS"]]
      file mkdir $innov_dir
      write_design -innovus -base_name $innov_dir/innovus
      write_reports -directory $innov_dir/ -tag INNOVUS
      report_module > $innov_dir/innovus_module.rpt 
    }}
  }
}
...
# RUN_DRC
################################################################################
create_flow_step -name run_drc -owner flow {
  if {[get_flow_config enable_signoff_drc]} {
    set design_n [get_flow_config design_name]
    exec ./scripts/shell/run_drc.csh $design_n
  }
}
...
# RUN_LVS
################################################################################
create_flow_step -name run_lvs -owner flow {
  if {[get_flow_config enable_signoff_lvs]} {
    set design_n [get_flow_config design_name]
    exec ./scripts/shell/run_lvs.csh $design_n
  }
}
